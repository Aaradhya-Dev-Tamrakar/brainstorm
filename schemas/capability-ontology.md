...
| **Research Experiments, Specs & RFCs** | **20** | Formal architecture specs (`ARCH-SPEC-001` to `006`), RFCs (`ARCH-RFC-001` to `004`), invariants (`INV-EPI-001`, `INV-MEM-001`), empirical sweeps (`EXP-001`, `EXP-DRIVE-SYNC-001`, `EXP-FUSION360-MCP-001` (dual-server: Autodesk native + custom bridge), `EXP-CLASSROOM-MCP-001`, `FLEET-001`, `GPU_RAM_ARCHITECTURE_SPEC`), and composition benchmarks (`COMPOSE-001`, `COMPOSE-002`). |
